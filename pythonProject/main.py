...
app.config['MAIL_USERNAME'] = 'bf05f15c465f10'
app.config['MAIL_PASSWORD'] = 'fde4fd4858e24e'
...
@app.route('/not_found')
def not_found():
    return jsonify(message='Nikal lo'), 404


@app.route('/my_first_get')
def my_first_get():
...
def retrieve_password(email: str):
    ...
    if user:
        msg = Message("your password is " + user.password,
                      sender="salman@admin.com",
                      recipients=[email])
        mail.send(msg)
        return jsonify(message="Password sent to " + email)
    else:
        return jsonify(message="User not found for email: " + email), 401
